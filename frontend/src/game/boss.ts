...
export const createBossRingConfig = (
  ...
  count: Math.max(7, Math.floor((side === 'boss' ? level.bossRings : level.playerRings) * 0.72)),
  minCount: 7,
  countGrowth: 0.18,
  difficultyGrowth: 0.045 + level.rank * 0.004,
  gapShrinkPerPhase: 0.006,
  minGapSize: Math.PI / 6.8,
  minSpacing: 8,
  safeStartRadius: 48,
  innerRadius: 34,
  outerRadius: size / 2 - 14,
  baseRotationSpeed: level.rotationSpeed * 0.72 * (side === 'boss' ? level.bossSpeedMultiplier : 1),
  baseHp: level.baseHp * (side === 'boss' ? level.bossDamageMultiplier : 1),
  baseGapSize: (Math.PI / (side === 'boss' ? 2.85 : 2.65)) * GAMEPLAY_TUNING.boss.gapScale,
  baseThickness: 4,
  closingSpeed: level.closingSpeed * 0.58 * (side === 'boss' ? level.bossShrinkMultiplier : 1),
  colors,
