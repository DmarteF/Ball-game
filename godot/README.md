...
| Perfil | `frontend/app/profile.tsx`, `frontend/src/components/ProfileAvatar.tsx`, `frontend/src/components/SkinIcon.tsx`, `frontend/src/components/UiIcon.tsx`, `frontend/src/game/skins.ts`, `frontend/src/game/achievements.ts`, `frontend/src/game/upgrades.ts` | `scenes/Profile.tscn`, `scripts/ProfileScreen.gd` | Ajustado nesta etapa | Configuracoes, audio, idioma, FPS e Hz removidos do Perfil. Tela agora fica focada em nome, nivel, XP, recursos, estatisticas e habilidades; dados reais ainda mockados ate o save completo ser portado. |
...
| Loja | `frontend/app/store.tsx`, assets `assets/icons/products/*`, `assets/ui/ui_store.png` | `scenes/Shop.tscn`, `scripts/VisualFeatureScreen.gd` | Visual criado | Cards e botoes mockados; sem compra real, anuncios reais ou entrega de recompensas. |
| Inventario | `frontend/app/inventory.tsx`, assets de baus/chaves em `assets/ui` | `scenes/Inventory.tscn`, `scripts/VisualFeatureScreen.gd` | Visual criado | Baus, chaves e itens sao exibidos visualmente; abertura real fica pendente. |
...
| Evento | `frontend/app/events.tsx`, `frontend/src/game/retention.ts`, `assets/ui/ui_event.png` | `scenes/Event.tscn`, `scripts/VisualFeatureScreen.gd` | Visual criado | Evento e tempo restante mockados; calendario/tempo real pendente. |
| Roleta | `frontend/app/wheel.tsx`, `assets/ui/ui_wheel.png` | `scenes/Wheel.tscn`, `scripts/VisualFeatureScreen.gd` | Visual criado | Roleta parada visual; sorteio real e custos pendentes. |
