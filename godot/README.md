...
| Skins | `frontend/app/transformations.tsx`, `frontend/src/game/skins.ts`, `frontend/src/components/SkinIcon.tsx`, `assets/skins/*` | `scenes/Skins.tscn`, `scripts/SkinsScreen.gd` | Visual criado | Filtros, progresso por raridade, grade de cards, raridades, imagens reais e botoes mockados. Equipar/evoluir/desbloquear ainda nao funcionam. Skins extras importadas aparecem com nome inferido quando a descricao literal ainda nao foi portada. |
| Upgrades/Melhorias | `frontend/app/upgrade-shop.tsx`, `frontend/src/game/upgrades.ts`, `frontend/src/game/balance.ts`, `frontend/src/components/UpgradeIcon.tsx` | `scenes/Upgrades.tscn`, `scripts/UpgradesScreen.gd` | Visual criado | Lista visual de upgrades permanentes, moeda, cards, bloqueios e botoes de custo. Compra/aplicacao real ainda pendente. |
